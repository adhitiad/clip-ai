...
def analyze_video_layout(video_path: str):
    ...
    def clamp_x(x):
        if x - half_crop_w < 0:
            return half_crop_w
        elif x + half_crop_w > width:
            return width - half_crop_w
        return x
...
def detect_active_speaker(video_path: str, split_x: list):
    ...
    with mp_face_mesh.FaceMesh(
        max_num_faces=2, min_detection_confidence=0.3
    ) as face_mesh:
        frame_idx = 0
        while True:
            ...
            if results.multi_face_landmarks:
                for landmarks in results.multi_face_landmarks:
                    ...
                    if abs(xc - split_x[0]) < abs(xc - split_x[1]):
                        if dist > 0.005:
                            s0_times.append(time_sec)
                    else:
                        if dist > 0.005:
                            s1_times.append(time_sec)
            frame_idx += frame_step
    cap.release()

    def format_expr(times):
        if not times:
            return "between(t,-1,-1)"
        ranges = []
        start = times[0]
        prev = times[0]
        for t in times[1:]:
...
def generate_dynamic_subtitles(words, video_width, video_height):
    ...
    for i, word in enumerate(words):
        ...
        txt = (
            ...
            .set_duration(max(0.1, word["end"] - word["start"]))
            .set_position(("center", video_height * 0.7))
        )
        clips.append(txt)
    return clips
...
def process_clip(video_url: str, clip_metadata: dict, index: int, download_func):
    ...
    if mode == "crop":
        v_stream = v_stream.filter("crop", w="ih*9/16", h="ih", x=smart_x, y=0).filter(
            "scale", 720, 1280
        )
    elif mode == "split":
        t0 = v_stream.filter("crop", w="ih*9/16", h="ih", x=smart_x[0], y=0).filter(
            "scale", 720, 640
        )
        t1 = v_stream.filter("crop", w="ih*9/16", h="ih", x=smart_x[1], y=0).filter(
            "scale", 720, 640
        )
        v_stream = ffmpeg.filter([t0, t1], "vstack")
    else:
        v_stream = v_stream.filter(
            "crop", w="ih*9/16", h="ih", x="iw/2-ow/2", y=0
        ).filter("scale", 720, 1280)

    # Render base vertical
    (
        ffmpeg.output(
            v_stream, temp_vertical_v1, vcodec="libx264", preset="ultrafast"
        ).run(quiet=True, overwrite_output=True)
    )
    ...
    if index == 0:  # Hanya hook awal
        ...
        if download_broll(clip_metadata.get("broll_query", "motivation"), broll_path):
            broll = (
                VideoFileClip(broll_path)
                .resize(height=1280)
                .crop(x_center=video_clip.w / 2, width=720)
                .set_duration(3)
                .fadeout(0.5)
            )
    ...
    final.write_videofile(
        final_variant_output, codec="libx264", audio_codec="aac", fps=30, logger=None
    )
