...
def init_vector_store():
    ...
    if not PINECONE_API_KEY or not HF_TOKEN:
        logger.warning(
            "⚠️ PINECONE_API_KEY atau HUGGINGFACEHUB_API_TOKEN belum diset. Vector Search dinonaktifkan."
        )
        return
    ...
    try:
        ...
        if PINECONE_INDEX_NAME not in existing_indexes:
            logger.info(
                f"Index {PINECONE_INDEX_NAME} tidak ditemukan, membuat index baru..."
            )
            pc.create_index(
        ...
        logger.error(f"⚠️ Gagal inisialisasi Pinecone: {e}")
...
def upsert_clip_vector(clip_id: int, topic: str, title: str, desc: str):
    ...
    try:
        ...
        logger.error(f"Gagal upsert vektor: {e}")
...
def search_similar_clips(query: str, top_k=3):
    ...
    try:
        ...
        logger.error(f"Gagal mencari vektor semantik: {e}")
        return []
